...
<head runat="server">
    <title>通过 HTML Form 修改用户密码</title>
</head>
<body>
    ...
    <%  
        ...
        // ========== ========== ========== ==========
        // 下方代码创建一个 HTML Form 提交数据后修改密码
        // (提交数据时将自动从 Session 中获取并向表单填充 user_guid 的值)
        // 修改密码后会自动向数据表 user_password_log 添加旧密码的记录
        // ========== ========== ========== ==========
        
        MySqlParameter[] user_info_update_password_mySqlParameters = new MySqlParameter[3];

        user_info_update_password_mySqlParameters[0] = new MySqlParameter("@user_guid", MySqlDbType.VarChar);
        user_info_update_password_mySqlParameters[0].Value = session_user_guid;

        user_info_update_password_mySqlParameters[1] = new MySqlParameter("@user_login_name", MySqlDbType.VarChar);
        user_info_update_password_mySqlParameters[1].Value = "user002";
        
        user_info_update_password_mySqlParameters[2] = new MySqlParameter("@user_login_password", MySqlDbType.VarChar);
        user_info_update_password_mySqlParameters[2].Value = "12345678";

        user_info_update_password[] user_info_update_password_s = DbHelper.GetModelData<user_info_update_password>(user_info_update_password_mySqlParameters);

        if (user_info_update_password_s != null)
        {
            Response.Write(ModelMethod.GetFormHTMLByModel(typeof(user_info_update_password), PageProcessMode.DataUpdate, user_info_update_password_s[0].id_userinfo, true));
        }
